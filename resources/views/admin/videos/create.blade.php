...
@section('content')
    ...
    <div class="m-portlet">
        ...
        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right" action="{{route('admin.videos.create')}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="m-portlet__body">
                ...
                <div class="form-group m-form__group @error('title') has-danger @enderror">
					...
					<select name="title" id="title" class="form-control" required>
						...
						<option value="{{$videoCategory->id}}" {{ old('title') == $videoCategory->id ? 'selected' : '' }}>{{$videoCategory->title}}</option>
						@endforeach
					</select>
                    @error('title')
				...
				<div class="form-group m-form__group @error('sub_title') has-danger @enderror">
                    ...
                    <input type="text" class="form-control m-input" name="sub_title" value="{{ old('sub_title') }}" placeholder="Sub Title" required="required">
                    @error('sub_title')
                    <div class="form-control-feedback">{{ $message }}</div>
                    @enderror
                </div>
				...
				<div class="form-group m-form__group @error('content') has-danger @enderror">
					...
					<textarea class="form-control" id="editor2" name="content" placeholder="Content" rows="6" required="required">{{ old('content') }}</textarea>
                    @error('content')
                    <div class="form-control-feedback">{{ $message }}</div>
                    @enderror
