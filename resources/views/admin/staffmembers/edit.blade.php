...
@section('content')
    ...
    <div class="m-portlet">
        ...
        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right" action="{{route('admin.staffmembers.edit', [$staff->id])}}" method="post" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="m-portlet__body">
                ...
                <div class="form-group m-form__group row @error('role') has-danger @enderror">
                    ...
                    <div class="col-lg-7 col-md-7 col-sm-12">
                        <select name="role" id="role" class="form-control" required>
                            ...
                            <option value="{{$role->id}}" {{ old('role', $staff->is_role) == $role->id ? 'selected' : '' }}>{{$role->name}}</option>
                            @endforeach
                        </select>
                        @error('role')
                ...
                <div class="form-group m-form__group row @error('name') has-danger @enderror">
                    ...
                    <div class="col-lg-7 col-md-7 col-sm-12">
                        <input type="text" class="form-control m-input" name="name" value="{{ old('name', $staff->name) }}" placeholder="Name" required autofocus>
                        @error('name')
                        <div class="form-control-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group m-form__group row @error('email') has-danger @enderror">
                    ...
                    <div class="col-lg-7 col-md-7 col-sm-12">
                        <input type="email" class="form-control m-input" name="email" value="{{ old('email', $staff->email) }}" placeholder="Email" required autocomplete="email">
                        @error('email')
                        <div class="form-control-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group m-form__group row @error('phone_no') has-danger @enderror">
                    ...
                    <div class="col-lg-7 col-md-7 col-sm-12">
                        <input type="text" class="form-control m-input" value="{{ old('phone_no', $staff->phone_no) }}" name="phone_no" placeholder="Phone No" autocomplete="phone_no" onkeypress="return NumericValidation(event);" maxlength="10" required>
                        @error('phone_no')
                        <div class="form-control-feedback">{{ $message }}</div>
                        @enderror
                    </div>
