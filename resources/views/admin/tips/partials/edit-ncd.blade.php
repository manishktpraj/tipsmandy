	...
	<div id="ncdcontainer" class="ncdcontainer" style="display: {{ $tip->segment == 'NCD' ? '' : 'none' }};">
        ...
        <div class="form-group m-form__group row @error('type_of_bond') has-danger @enderror">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12">
                <input type="text" class="form-control m-input" name="type_of_bond" value="{{ old('type_of_bond', $tip->type_of_bond ?? '') }}" placeholder="Type of Bond">
                @error('type_of_bond')
        ...
        <div class="form-group m-form__group row">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12">
                <table class="table">
                    ...
                    <tbody>
                        @php
                            $ncdInvesmentCount = count($tip->ncdInvestments);
                            $exitncdInvesment = 4-$ncdInvesmentCount;
                        @endphp
                        @foreach($tip->ncdInvestments as $row_ncd)
                        <tr>
                            <td>
                                <input type="text" class="form-control" name="ncd_investment[]" value="{{$row_ncd->investment}}" placeholder="Investment" onkeypress="return NumericValidation(event);" />
                            </td>
                            <td>
                                <input type="text" class="form-control" name="ncd_duration[]" value="{{$row_ncd->duration}}" placeholder="Duration" onkeypress="return NumericValidation(event);"  />
                            </td>
                            <td>
                                <input type="text" class="form-control" name="ncd_maturity_amount[]" value="{{$row_ncd->maturity_amount}}" placeholder="Maturity Amount" onkeypress="return NumericValidation(event);"  />
                            </td>
                        </tr>
                        @endforeach

                        @for ($ncdIn = 1; $ncdIn < $exitncdInvesment; $ncdIn++)
                         <tr>
                            <td>
        ...
        <div class="form-group m-form__group row @error('unit_price') has-danger @enderror">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12">
                <input type="text" class="form-control m-input" name="unit_price" value="{{ old('unit_price', $tip->unit_price ?? '') }}" placeholder="Unit Price" id="unit_price" onkeypress="return NumericValidation(event);">
                @error('unit_price')
                <div class="form-control-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-group m-form__group row @error('ncd_rating') has-danger @enderror">
            ...
            <div class="col-lg-7 col-md-7 col-sm-12">
                <select class="form-control" name="ncd_rating" id="ncd_rating">
                    ...
                    <option {{ old('ncd_rating', $tip->rating ?? '') == $ncdRating ? 'selected' : '' }} value="{{$ncdRating}}">{{$ncdRating}}</option>
                    @endforeach
                </select>
                @error('ncd_rating')
                <div class="form-control-feedback">{{ $message }}</div>
