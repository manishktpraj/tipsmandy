                    ...
                    <div class="col-lg-7 col-md-7 col-sm-12">
                        <table class="table">
                            ...
                            <tbody class="tips_featured_wrapper">
                              @foreach($plan->planfeatureds as $planfeatured)
                                <input type="hidden" name="plan_featureds_value_id[]" value="{{$planfeatured->id}}" >
                                <tr id="remove_tips_featured_wrapper">
                                    <td>
                                        <input type="file" class="form-control" name="update_featured_images[{{$planfeatured->id}}]" accept="image/*" />
                                        <span class="m-form__help">Allowed formats - jpg, jpeg, png.</span>
                                    </td>
                                    <td>
                                        <input type="hidden" name="plan_featureds_id[]" value="{{$planfeatured->id}}" >
                                        <input type="text" class="form-control" name="update_featured_name[]" value="{{$planfeatured->name}}" placeholder="Demo Text 1" required />
                                    </td>
                                    <td><button type="button" id="btn_remove_featured" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only" style="margin-top: 6px;"><i class="la la-trash-o"></i></button></td>
                                </tr>
                              @endforeach
                            </tbody>
                        </table>
